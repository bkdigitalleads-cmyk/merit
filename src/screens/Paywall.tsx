...
const FEATURES: { icon: string; title: string; sub: string; free?: boolean }[] = [
  { icon: '🤝', title: 'Log a shift in seconds', sub: 'Date, hours, organization, and who supervised.', free: true },
  { icon: '📄', title: 'Verification-ready PDF', sub: 'Official-style hours log with totals and a supervisor signature line.' },
  { icon: '♾️', title: 'Unlimited entries', sub: 'Free logs 10 shifts. Pro goes all the way to your goal.' },
...
export default function PaywallModal({ privacyUrl }: { privacyUrl: string }) {
  ...
  const buy = async () => {
    ...
    if (res.ok && res.isPro) {
      setIsPro(true);
      hidePaywall();
      Alert.alert('You’re all set', 'Every hour counts now, all the way to your goal.');
      ...
      <View style={[styles.container, { backgroundColor: theme.bg }]}>
        <ScrollView contentContainerStyle={styles.scroll}>
          ...
          <Text style={[styles.subtitle, { color: theme.textSecondary }]}>
            Start with {weeklyTrial ?? '3 days free'}. Log every volunteer shift, watch the requirement fill up, and hand your school a clean, signed hours log.
          </Text>
