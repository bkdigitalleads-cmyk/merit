...
const FEATURES: { icon: string; title: string; sub: string }[] = [
  { icon: '🤝', title: 'Unlimited entries', sub: 'Free logs 10 shifts. Pro goes all the way to your goal.' },
  { icon: '📄', title: 'Verification-ready PDF', sub: 'Official-style hours log with totals and a supervisor signature line.' },
  { icon: '🎯', title: 'Goal tracking', sub: 'NHS, college apps, scholarships — watch the requirement fill up.' },
  { icon: '🧾', title: 'CSV export', sub: 'Every entry as a spreadsheet, yours to keep.' },
  { icon: '🔒', title: 'Face ID lock', sub: 'Your service record stays your business.' },
];
...
export default function PaywallModal({ privacyUrl }: { privacyUrl: string }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setOffering(off);
      // Merit is time-boxed use — an NHS year or application season, not a
      // decade — so the one-time Lifetime purchase is the hero and default;
      // the small yearly subscription is the anchor for price-sensitive
      // families. The 10-entry free tier is the funnel, so there's no
      // separate time-limited trial.
      const lifetime =
  ...
  const buy = async () => {
    ...
    if (res.ok && res.isPro) {
      setIsPro(true);
      hidePaywall();
      Alert.alert('Welcome to Pro ✨', 'Every hour counts now — all the way to your goal.');
  ...
  const onRestore = async () => {
    ...
    if (res.ok && res.isPro) {
      setIsPro(true);
      hidePaywall();
      Alert.alert('Restored', 'Your Merit Pro purchase is active again.');
    } else if (res.ok) {
      Alert.alert(
        'No purchases found',
        'We couldn’t find a previous Merit Pro purchase on this Apple ID.',
      );
    } else {
      ...
      <View style={[styles.container, { backgroundColor: theme.bg }]}>
        <ScrollView contentContainerStyle={styles.scroll}>
          ...
          <Text style={[styles.subtitle, { color: theme.textSecondary }]}>
            Log every volunteer shift, watch the requirement fill up, and hand your
            school a clean, signed hours log — no lost paper forms.
          </Text>
        ...
        */}
        ...
        >
          <Text style={[styles.legalText, { color: theme.textSecondary }]}>
            Merit Pro — Lifetime{lifetimePrice ? ` ${lifetimePrice}` : ''}, a
